...
if __name__ == "__main__":
    ...
    Akashlibrary = Library(
        ["Let Us C++", "Let Us Python", "Artificial Intelligence", "Statics & Probility", "Technical Communication", "Discrete Structure"])
    student = Student()
    track = []

    print("\t\t\t\t\t\t\t♦♦♦♦♦♦♦ WELCOME TO THE AKASH LIBRARY CENTRE ♦♦♦♦♦♦♦\n")
    print(
    ...
    while (True):
        ...
        try:
            ...
            if usr_response == 1:  # listing
                Akashlibrary.displayAvailableBooks()
            elif usr_response == 2:  # borrow
                Akashlibrary.borrowBook(
                    input("Enter your name: "), student.requestBook())
            elif usr_response == 3:  # return
                Akashlibrary.returnBook(student.returnBook())
            elif usr_response == 4:  # donate
                Akashlibrary.donateBook(student.donateBook())
